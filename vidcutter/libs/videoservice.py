...
class VideoService(QObject):
    ...
    def cut(self, source: str, output: str, frametime: str, duration: str, allstreams: bool=True, vcodec: str=None,
        ...
        if run:
            ...
            png_frame = QFileInfo(output).fileName().replace(".webm", "%3d.png")
            png_dir = file_dir + 'tmp' + str(timestamp) + '/'
            gif_dir = file_dir + QFileInfo(output).fileName().replace(".webm", ".gif")
            ...
            if mp4Output:
                result = self.cmdExec(self.backends.ffmpeg, webmArgs.format(frametime, source, duration, QDir.fromNativeSeparators(output.replace(".webm", ".webm"))))
